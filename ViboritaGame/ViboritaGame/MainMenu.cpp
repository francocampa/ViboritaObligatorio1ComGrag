...
void MainMenu::loadLevels()
{
	...
	for (std::string levelFile:levelFiles) {
		...
		//if (!result) {
		//	printf("Error loading level: %s", levelFile);
		//	continue;
		//}


		pugi::xml_node levelNode = levelXMLReader.child("Level");
		if (!levelNode) {
			printf("Error loading level: %s. A Level node is required as root of the XML file", levelFile);
			continue;
		}
		...
		if(!viboritaNode) {
			printf("Error loading level: %s. A Vivorita node is required inside the main Level node", levelFile);
			continue;
		}
		if (!gridNode) {
			printf("Error loading level: %s. A Grid node is required inside the main Level node", levelFile);
			continue;
		}
